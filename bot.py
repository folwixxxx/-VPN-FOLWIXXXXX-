...
def process_payment(call, days, ton, stars, bal):
    ...
    keyboard.row(
        ...
        InlineKeyboardButton("⭐ Telegram Stars", callback_data=f"stars_{days}_{stars}")
    )
    keyboard.row(
...
def handle_balance_payment(call):
    ...
    bot.edit_message_text("⏳ **Создаю подписку...**\n\nПожалуйста, подождите.", call.message.chat.id, call.message.message_id, parse_mode='Markdown')
    link = create_subscription(user_id, days)
...
def handle_ton_payment(call):
    ...
    pending_payments[call.from_user.id] = {"days": days}
    keyboard = InlineKeyboardMarkup()
    keyboard.add(InlineKeyboardButton("✅ Я перевел", callback_data=f"check_{days}_{amount}"))
